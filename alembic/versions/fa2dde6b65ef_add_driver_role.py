...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE users ADD CONSTRAINT check_role CHECK (role IN ('admin', 'user', 'driver'))")

    op.add_column("orders", sa.Column("driver_id", sa.UUID(), nullable=True))

    op.create_foreign_key("fk_orders_driver_id", "orders", "users", ["driver_id"], ["id"])
