...
async def get_or_create_active_cart(session: AsyncSession, user_id: uuid.UUID) -> Cart:
...
async def get_cart_by_user_id(session: AsyncSession, user_id: uuid.UUID) -> Cart | None:
    result = await session.execute(
        select(Cart)
        .where(Cart.user_id == user_id, Cart.status == CartStatus.ACTIVE.value)
        .options(selectinload(Cart.items).selectinload(CartItem.product))
...
async def get_cart_by_id(session: AsyncSession, cart_id: uuid.UUID) -> Cart | None:
    result = await session.execute(select(Cart).where(Cart.id == cart_id))
    return result.scalar_one_or_none()


async def add_item_to_cart(
    session: AsyncSession, cart_id: uuid.UUID, item_data: CartItemCreate
) -> CartItem:
